  ...
  <section class="min-h-screen ">
    ...
    <div v-else-if="error || !user" class="flex min-h-screen items-center justify-center px-4">
      <div class="text-center">
        <div
          class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gray-100 dark:bg-neutral-800">
      ...
      <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div class="-mt-16 mb-6 flex flex-col gap-4 sm:-mt-20 sm:flex-row sm:items-end sm:justify-between">
          <div class="flex items-end gap-4">
            <UAvatar :src="user.profile_picture || user.user?.profile_picture || undefined"
              :alt="user.full_name || 'Member'" size="3xl"
            ...
            <div class="mb-2 space-y-1 z-30">
              ...
              <div class="flex flex-wrap gap-1.5 pt-1">
                ...
                <UBadge v-if="user.user?.is_active || user.is_active" color="neutral" variant="soft" size="sm"
                  class="rounded-full">
          ...
          <div class="flex flex-wrap gap-2 pb-2">
            <UButton v-if="user.github_link" :to="user.github_link" target="_blank" rel="noopener noreferrer"
              icon="i-lucide-github" color="neutral" variant="outline" size="sm" class="rounded-xl">
              GitHub
            </UButton>
            <UButton v-if="user.portfolio_website" :to="user.portfolio_website" target="_blank"
              rel="noopener noreferrer" icon="i-lucide-globe" color="primary" variant="soft" size="sm"
              class="rounded-xl">
              Portfolio
            </UButton>
          </div>
        ...
        <div class="grid gap-5 pb-12 lg:grid-cols-[300px_1fr]">
          ...
          <div class="space-y-4 w-full">
            ...
            <!-- Contact info -->
            <div
              class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900 w-full">
              <p class="mb-4 text-xs font-semibold uppercase tracking-widest text-gray-400">Contact</p>
              <div class="space-y-3 w-full">
                <div v-if="user.email || user.user?.email" class="flex items-start gap-3 md:max-w-[200px]">
                  <div
                    class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-primary-50 dark:bg-primary-900/20">
                  ...
                  <div class="min-w-0">
                    ...
                    <a :href="`mailto:${user.email || user.user?.email}`"
                      class="truncate text-sm text-gray-700 hover:text-primary-500 transition-colors dark:text-gray-300 w-[150px] lg:w-full">
                      {{ user.email || user.user?.email }}
                    </a>
                  </div>
                </div>

                <div v-if="user.phone_number" class="flex items-start gap-3 md:max-w-[200px]">
                  <div
                    class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-primary-50 dark:bg-primary-900/20">
                ...
                <div v-if="user.github_link" class="flex items-start gap-3 w-full md:max-w-[200px]">
                  <div
                    class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gray-50 dark:bg-neutral-800">
                  ...
                  <div class="min-w-0 w-full">
                    ...
                    <a :href="user.github_link" target="_blank" rel="noopener"
                      class="block truncate text-sm text-primary-500 hover:underline w-[150px] lg:w-full">
                      {{ formatUrl(user.github_link) }}
                    </a>
                  </div>
                </div>

                <div v-if="user.portfolio_website" class="flex items-start gap-3 w-full md:max-w-[200px]">
                  <div
                    class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gray-50 dark:bg-neutral-800">
            ...
            <div
              ...
              <div class="space-y-3">
                <div class="flex items-start gap-3">
                  <div
                    class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-violet-50 dark:bg-violet-900/20">
          ...
          <div class="space-y-5">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <div class="flex flex-wrap gap-2">
                    <UBadge v-for="skill in parseList(user.technical_skills)" :key="skill" color="primary"
                      variant="soft" size="md" class="rounded-full px-3">
                      {{ skill }}
                    </UBadge>
                ...
                <div class="border-t border-gray-100 pt-5 dark:border-neutral-800">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <UBadge v-for="interest in parseList(user.areas_of_interest)" :key="interest" color="neutral"
                      variant="soft" size="md" class="rounded-full px-3">
                      {{ interest }}
                    </UBadge>
            ...
            <div
              ...
              <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div v-for="item in activityItems" :key="item.label"
                  ...
                  <div
                    class="mb-2 flex h-7 w-7 items-center justify-center rounded-lg bg-white shadow-sm dark:bg-neutral-700">
            ...
            <div v-if="(user.created_projects?.length || 0) > 0"
              class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
              <div class="mb-5 flex items-center gap-2">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">
                This member has contributed to <span class="font-semibold text-gray-800 dark:text-gray-200">{{
                  user.created_projects?.length }}</span>
                project{{ (user.created_projects?.length || 0) !== 1 ? 's' : '' }} in the MuTech community.
              </p>
            </div>

            <!-- Events attended -->
            <div v-if="(user.event_attendances?.length || 0) > 0"
              class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
              <div class="mb-5 flex items-center gap-2">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">
                Attended <span class="font-semibold text-gray-800 dark:text-gray-200">{{ user.event_attendances?.length
                  }}</span>
