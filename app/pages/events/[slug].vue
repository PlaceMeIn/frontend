<template>
  <div class="min-h-screen bg-gray-50 dark:bg-neutral-950">
    ...
    <div
      ...
      <div class="max-w-7xl mx-auto px-3 sm:px-6">
        <div class="flex items-center justify-between h-14">
          <div class="lg:hidden">
            <UButton to="/events" color="neutral" variant="ghost" size="sm" class="!p-1.5">
          ...
          <div class="flex items-center gap-1.5">
            <UButton v-if="eventData?.status" :icon="getIcon(eventData?.status)" size="sm"
              :color="getColor(eventData?.status)" :variant="eventData?.status === 'completed' ? 'outline' : 'solid'"
              :disabled="eventData?.status === 'ongoing' ||
                eventData?.status === 'completed'
                " :label="getLabel(eventData?.status, eventData?.start_date || '')"
              :to="eventData?.registration_link || '#'" target="_blank" class="w-full" />

            <UButton color="neutral" variant="soft" size="sm" class="!px-2.5 !py-1.5" @click="shareContent">
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
      <OnThisPage :sections="sections" class="mb-4" :class="{ 'opacity-50 pointer-events-none': pending || error }" />

      <div v-if="pending" class="flex justify-center items-center min-h-[50vh]">
        <div class="text-center">
          <UIcon name="i-lucide-loader-circle" class="w-10 h-10 text-primary animate-spin mx-auto mb-3" />
          <p class="text-sm text-gray-600 dark:text-gray-400">Loading...</p>
        </div>
      </div>

      <div v-else-if="error" class="flex flex-col items-center justify-center min-h-[50vh] gap-3">
        ...
        <p class="text-gray-600 dark:text-gray-400 text-center max-w-md text-xs">
      ...
      <div v-else-if="eventData" class="space-y-4">
        ...
        <section id="overview" class="scroll-mt-16">
          <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm overflow-hidden">
            <div class="relative h-[250px] sm:h-[300px] lg:h-[350px]">
              <img :src="eventData.image || eventData.image_url || '/placeholder.jpg'
                " :alt="eventData.title" class="w-full h-full object-cover" />
              <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />

              <div class="absolute top-3 left-3 flex flex-wrap gap-1.5">
                <UBadge :color="getStatusColor(eventData.status)" variant="solid"
                  class="!px-2 !py-1 text-[10px] font-medium backdrop-blur-sm">
                  <UIcon :name="getStatusIcon(eventData.status)" class="w-3 h-3 mr-1" />
                  {{ eventData.status_display || eventData.status }}
                </UBadge>

                <UBadge v-if="eventData.is_featured" color="warning" variant="solid"
                  class="!px-2 !py-1 text-[10px] font-medium backdrop-blur-sm">
              ...
              <div class="absolute top-3 right-3">
                <UBadge color="white" variant="solid"
                  ...
                  <UIcon :name="getEventTypeIcon(eventData.event_type)" class="w-3 h-3 mr-1" />
              ...
              <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
                ...
                <p class="text-sm sm:text-base text-gray-200 flex items-center gap-1">
            ...
            <div
              ...
              <div class="p-3">
                <div class="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p class="text-[10px] text-primary mt-0.5 flex items-center gap-0.5">
              ...
              <div class="p-3">
                <div class="flex items-start gap-2">
                  <div class="p-1.5 bg-primary/10 rounded">
                    <UIcon name="i-lucide-map-pin" class="w-3.5 h-3.5 text-primary" />
                  </div>
                  <div>
                    <p class="text-[10px] text-gray-500 dark:text-gray-400">
                      Location
                    ...
                    <p v-if="eventData?.venue" class="text-[10px] text-gray-600 dark:text-gray-300 truncate">
                      {{ eventData?.venue }}
                    </p>
                  </div>
                </div>
              ...
              <div class="p-3">
                <div class="flex items-start gap-2">
                  <div class="p-1.5 bg-primary/10 rounded">
                    <UIcon name="i-lucide-users" class="w-3.5 h-3.5 text-primary" />
                  </div>
                  <div>
                    <p class="text-[10px] text-gray-500 dark:text-gray-400">
                      Attendees
              ...
              <div class="p-3">
                <div class="flex items-start gap-2">
                  <div class="p-1.5 bg-primary/10 rounded">
                    <UIcon name="i-lucide-door-open" class="w-3.5 h-3.5 text-primary" />
                  </div>
                  <div>
                    <p class="text-[10px] text-gray-500 dark:text-gray-400">
                      Capacity
          ...
          <div class="lg:col-span-2 space-y-4">
            ...
            <section id="description" class="scroll-mt-16">
              <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-4">
                <h2 class="text-base font-semibold mb-2 flex items-center gap-1.5">
            ...
            <!-- Speakers -->
            <section id="speakers" v-if="eventData.speakers?.length" class="scroll-mt-16">
              <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-4">
                <h2 class="text-base font-semibold mb-3 flex items-center gap-1.5">
                  <div class="p-1 bg-primary/10 rounded">
                    <UIcon name="i-lucide-mic-2" class="w-3.5 h-3.5 text-primary" />
                  </div>
                  Speakers ({{ eventData.speakers.length }})
                </h2>

                <div class="space-y-3">
                  <div v-for="speaker in eventData.speakers" :key="speaker.id"
                    class="flex gap-3 p-2 bg-gray-50 dark:bg-neutral-800/50 rounded-lg">
                    <div class="relative w-12 h-12 flex-shrink-0">
                      <img :src="speaker.image || '/placeholder-avatar.jpg'" :alt="speaker.name"
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <p v-if="speaker.company" class="text-gray-600 dark:text-gray-400 text-xs">
                        {{ speaker.company }}
                      </p>
                      <p v-if="speaker.topic" class="text-xs text-gray-700 dark:text-gray-300 mt-1 line-clamp-2">
          ...
          <div class="space-y-4">
            ...
            <section id="details" class="scroll-mt-16">
              <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-4">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex items-start gap-2">
                    <UIcon name="i-lucide-calendar" class="w-3.5 h-3.5 text-primary mt-0.5" />
                    <div>
                  ...
                  <div class="flex items-start gap-2">
                    <UIcon name="i-lucide-calendar-check" class="w-3.5 h-3.5 text-primary mt-0.5" />
                    <div>
                  ...
                  <div class="flex items-start gap-2">
                    <UIcon name="i-lucide-map-pin" class="w-3.5 h-3.5 text-primary mt-0.5" />
                    <div>
            ...
            <section id="registration" class="hidden lg:block scroll-mt-16">
              <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-4">
                <h3 class="text-sm font-semibold mb-3 flex items-center gap-1.5">
                  <div class="p-1 bg-primary/10 rounded">
                    <UIcon name="i-lucide-ticket" class="w-3.5 h-3.5 text-primary" />
                  </div>
                  Registration
                </h3>
            ...
            <section id="share" class="scroll-mt-16">
              <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-4">
                ...
                <div class="flex flex-wrap gap-2">
                  <UButton icon="i-lucide-link" color="neutral" variant="soft" size="xs"
                    @click="copyToClipboard(currentUrl)">
                    Copy
                  </UButton>

                  <UButton icon="i-lucide-facebook" color="neutral" variant="soft" size="xs" square
                    :to="`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(currentUrl)}`"
                    target="_blank" aria-label="Share on Facebook" />

                  <UButton icon="i-lucide-linkedin" color="neutral" variant="soft" size="xs" square
                  ...
                  <UButton icon="i-simple-icons-whatsapp" color="neutral" variant="soft" size="xs" square
                    :to="`https://wa.me/?text=${encodeURIComponent(shareText)}`" target="_blank"
            ...
            <template #content="{ item }">
              ...
              <section id="takeaways">
                <div v-if="item.key === 'takeaways'">
                  <UModal fullscreen :overlay="false" title="Mut Tech Club Takeaways" description="Key take aways">
                    <UButton icon="i-lucide-expand" color="neutral" variant="subtle" />

                    <template #body>
                      <TakeAways :id="eventData?.id" :type="'event'" class="h-full" />
                    </template>
                  </UModal>
                  <TakeAways :id="eventData?.id" :type="'event'" class="h-[400px]" />
                </div>
              </section>
        ...
        <section id="related" class="scroll-mt-16">
          <HighlightUpcomingEvent :limit="10" :h-scroll="true" :related="eventData?.event_type"
            :title="'Related Events'" />
        </section>
      </div>

      <!-- Not Found -->
      <div v-else class="flex flex-col items-center justify-center min-h-[50vh] gap-3">
        <div class="w-16 h-16 bg-gray-100 dark:bg-neutral-800 rounded-full flex items-center justify-center">
